...
# magic
prompt_enable = True
def attention_pool(tensor, pool, thw_shape, magic_number, has_cls_embed=True, norm=None):
    ...
    if has_cls_embed:
        if prompt_enable:
            cls_tok, prompt, tensor = tensor[:, :, :1, :], tensor[:, :, 1:magic_number, :], tensor[:, :, magic_number:, :]
        else:
    ...
    if has_cls_embed:
        if prompt_enable:
            tensor = torch.cat((cls_tok, prompt, tensor), dim=2)
        else:
...
def cal_rel_pos_spatial(
    attn, q, k, has_cls_embed, q_shape, k_shape, rel_pos_h, rel_pos_w, magic_number
):
    """
    ...
    global prompt_enable
    if prompt_enable:
        sp_idx = magic_number if has_cls_embed else 0
    else:
...
def cal_rel_pos_temporal(attn, q, has_cls_embed, q_shape, k_shape, rel_pos_t, magic_number):
    """
...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        separate_qkv=False,
        num_of_prompt_tokens=1
    ):
        super().__init__()
        self.num_of_prompt_tokens = num_of_prompt_tokens
...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        self.attn = MultiScaleAttention(
            ...
            num_of_prompt_tokens=num_prompt_tokens
        )
        self.num_of_prompt_tokens = num_prompt_tokens
        self.drop_path = (
    ...
    def forward(self, x, thw_shape=None):
        ...
        x_res, _ = attention_pool(
            x, self.pool_skip, thw_shape, self.num_of_prompt_tokens, has_cls_embed=self.has_cls_embed
        )
