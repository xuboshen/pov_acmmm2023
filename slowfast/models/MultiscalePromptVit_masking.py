...
from slowfast.models.three_d_trl import ThreeDTRL
...
'''
    useful components: self.patch_embed\self.head\self.blocks
...
class RandomPatchPrompter(nn.Module):
    def __init__(self, cfg):
        ...
        # cfg.PATCHPROMPT.IMAGE_SIZE
        self.isize = 224
        # 40
        self.num_segments = cfg.TRAIN.NUM_SEGMENTS
        self.hand_psize = cfg.PATCHPROMPT.PROMPT_SIZE
        self.object_psize = cfg.PATCHPROMPT.PROMPT_SIZE
        self.psize = cfg.PATCHPROMPT.PROMPT_SIZE
        # self.patch_prompt = nn.ParameterList([nn.Parameter(torch.randn([1, 3, cfg.TRAIN.NUM_SEGMENTS, self.hand_psize, self.hand_psize])), # left hand 
        #             nn.Parameter(torch.randn([1, 3, cfg.TRAIN.NUM_SEGMENTS, self.hand_psize, self.hand_psize])), # right hand
        #             nn.Parameter(torch.randn([1, 3, cfg.TRAIN.NUM_SEGMENTS, self.hand_psize, self.hand_psize]))]) # object
        self.patch_prompt = [nn.Parameter(torch.randn([1, 3, cfg.TRAIN.NUM_SEGMENTS, self.hand_psize, self.hand_psize])), # left hand 
            nn.Parameter(torch.randn([1, 3, cfg.TRAIN.NUM_SEGMENTS, self.hand_psize, self.hand_psize])), # right hand
            nn.Parameter(torch.randn([1, 3, cfg.TRAIN.NUM_SEGMENTS, self.hand_psize, self.hand_psize]))] # object
    def forward(self, x, boxes=None, idx=None):
        '''
        boxes: B, T, 4
        '''
        # self.patch_prompt = [nn.Parameter(torch.randn([1, 3, self.num_segments, self.hand_psize, self.hand_psize])), # left hand 
        #     nn.Parameter(torch.randn([1, 3, self.num_segments, self.hand_psize, self.hand_psize])), # right hand
        #     nn.Parameter(torch.randn([1, 3, self.num_segments, self.hand_psize, self.hand_psize]))] # object
        if idx == 0 or idx == 1:
            psize = self.hand_psize
        elif idx == 2:
            psize = self.object_psize
        else:
            psize = self.psize
        ...
        if boxes is not None:
            center = [(boxes[:, :, 0] + boxes[:, :, 2]) // 2 - 16, (boxes[:, :, 1] + boxes[ :, :, 3]) // 2 - 16]
                # B, T
            x_ = center[0]
            y_ = center[1]
            # min_center_x = torch.min(x_, dim=1)[0]
            # min_center_y = torch.min(y_, dim=1)[0]
            # max_center_x = torch.max(x_, dim=1)[0]
            # max_center_y = torch.max(y_, dim=1)[0]
            prompt = torch.zeros([B, 3, 4, self.isize, self.isize]).cuda()
            left = F.relu(x_ - psize // 2)
            # right = - F.relu(self.isize - (x_ + psize // 2)) + self.isize
            upper = F.relu(y_ - psize // 2)
            # lower = - F.relu(self.isize - (y_ + psize // 2)) + self.isize
            for b in range(B):
                for t in range(T):
                    # print(right[b,t] - left[b, t] + 1, lower[b, t] - upper[b, t] + 1)
                    if left[b, t] + self.psize >= 224:
                        left[b, t] = 224 - self.psize
                    if upper[b, t] + self.psize >= 224:
                        upper[b, t] = 224 - self.psize
                    prompt[b, :, t, left[b, t]: left[b, t] + self.psize, upper[b, t]: upper[b, t] + self.psize] += self.patch_prompt[idx][0, :, t, :, :].cuda()

            return x + prompt
        else:
            prompt = torch.zeros([1, 3, 4, self.isize, self.isize]).cuda()
            x_ = np.random.choice(3) * (self.isize - self.psize) // 3
            y_ = np.random.choice(3) * (self.isize - self.psize) // 3
            prompt[:, :, :, x_:x_ + self.psize, y_:y_ + self.psize] += self.patch_prompt[0]
            x_ = np.random.choice(3) * (self.isize - self.psize) // 3
            y_ = np.random.choice(3) * (self.isize - self.psize) // 3
            prompt[:, :, :, x_:x_ + self.psize, y_:y_ + self.psize] += self.patch_prompt[1]
            x_ = np.random.choice(3) * (self.isize - self.psize) // 3
            y_ = np.random.choice(3) * (self.isize - self.psize) // 3
            prompt[:, :, :, x_:x_ + self.psize, y_:y_ + self.psize] += self.patch_prompt[2]
            return x + prompt
@MODEL_REGISTRY.register()
class MPromptViT_masking(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.PROMPT.INITIALIZATION == 'random':
            ...
            if self.cfg.PROMPT.DEEP:
                ...
                for idx_emb, (layer_i, multiple) in enumerate(self.cfg.PROMPT.DIM_MUL):
                    val = math.sqrt(6. / float(3 * reduce(mul, self.patch_dims, 1) + dimension))
                    if self.specific_prompt_enable:
                    # try:
                        self.deep_specific_prompt_embeddings.append(nn.Parameter(torch.zeros(1, 4, self.num_prompt_tokens[layer_i - 1], int(dimension))))
        ...
        if cfg.PATCHPROMPT.ENABLE:
            self.patch_prompt = RandomPatchPrompter(cfg)
    ...
    def forward_deep_prompt(self, x, thw, domain_labels):
        ...
        for blk_idx, blk in enumerate(self.blocks):
            ...
            if blk_idx in self.cfg.PROMPT.BLOCK_IDX and self.cfg.PROMPT.DEEP:
                if self.specific_prompt_enable:
                    if not self.training:
                        # evaluation
                ...
                if self.specific_prompt_enable and not self.agnostic_prompt_enable:
                    ...
                    raise NotImplementedError("Not Implemented Prompt type")
            x, thw = blk(x, thw)
            if self.cfg.PROMPT.STAGE2 and self.cfg.PROMPT.SSDIV and self.cfg.PROMPT.DEEP:
    ...
    def forward(self, x, bboxes=None, return_attn=False, return_feat=False, alpha=None, domain_labels=None):
        '''
        bboxes: B, temp4, obj3, box4
        '''
        x = x[0]
        if self.cfg.PATCHPROMPT.ENABLE and bboxes is not None:
            if bboxes is not None:
                for box in range(3):
                    x = self.patch_prompt(x, bboxes[:, :, box, :], box)
            else:
                x = self.patch_prompt(x)
