...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    Args:
        ...
        split (str): the split of the data loader. Options include `train`,
            `val`, and `test`.
    """
    assert split in ["train", "val", "test", "target_domain", "target_domain_te"]
    if split in ["train"]:
        ...
        shuffle = False
        drop_last = False
    elif split in ["target_domain"]:
        dataset_name = cfg.TRAIN.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = True
        drop_last = False
    elif split in ["target_domain_te"]:
        dataset_name = cfg.TRAIN.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = False
        drop_last = False    
...
#             self.next_target = [t.cuda(non_blocking=True) for t in self.next_target]
