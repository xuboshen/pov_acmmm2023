...
def decoupleActions(outputs, labels, dataset='assembly101'):
    ...
    if 'assembly101' in dataset:
        ...
        action_mapping_filepath = 'subset_actions.csv'
...
def multitask_accuracy(outputs, labels, topk=(1,)):
    ...
    for output, label in zip(outputs, labels):
        _, max_k_idx = output.float().topk(max_k, dim=1, largest=True, sorted=True)
    ...
    for k in topk:
        ...
        # accuracies.append((-accuracy_at_k).add_(100))
...
class TrainMeter(object):
    ...
    def log_iter_stats(self, cur_epoch, cur_iter, state=None):
        """
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            stats["top1_{}_err".format(state)] = self.mb_top1_err.get_win_median()
            stats["top5_{}_err".format(state)] = self.mb_top5_err.get_win_median()
        logging.log_json_stats(stats)

    def log_epoch_stats(self, cur_epoch, state=None):
        """
        Log the stats of the current epoch.
        Args:
            cur_epoch (int): the number of current epoch.
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            ...
            stats["top1_{}_err".format(state)] = top1_err
            stats["top5_{}_err".format(state)] = top5_err
            stats["loss"] = avg_loss
        logging.log_json_stats(stats)
...
class ValMeter(object):
    ...
    def update_predictions(self, preds, labels, multitask=False):
        """
        ...
        if not multitask:
            ...
            self.all_vpreds.append(preds[0])
            self.all_vlabels.append(labels[0])
            self.all_npreds.append(preds[1])
            self.all_nlabels.append(labels[1])
