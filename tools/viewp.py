    ...
    for cur_iter, (inputs, labels, boxes) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.PROMPT.ENABLE:
                ...
                domain_label_idx = torch.randperm(labels[1].shape[0])
                preds = model(inputs, domain_labels=labels[1], bboxes=boxes)
                shuffled_preds = model(inputs, domain_labels=labels[1][domain_label_idx].view(labels[1].size()), bboxes=boxes)
                ...
                preds = model(inputs)
            preds = preds[0]
            shuffled_preds = shuffled_preds[0]
            loss1 = 0.0001 * loss_fun(shuffled_preds, preds) / cfg.TRAIN.GRADIENT_ACCUMULATION
            loss2 = loss_fun(preds, labels) / cfg.TRAIN.GRADIENT_ACCUMULATION
            loss = loss1 + loss2
            loss_list += [loss1, loss2, loss]
