...
class GroupColorJitter(object):
    def __init__(self, brightness=0.5, hue=0.3):
        # self.worker = torchvision.transforms.ColorJitter(brightness, hue)
        self.worker = torchvision.transforms.ColorJitter(0.4, 0.4, 0.4)
    def __call__(self, img_group):
        return [self.worker(img) for img in img_group]
...
class GroupRandomRotation(object):
    def __init__(self, degree):
        self.worker = torchvision.transforms.RandomRotation(degree)
...
class GroupGray(object):
    ...
    def __call__(self, img_group):
        return [self.worker(img).convert("RGB") for img in img_group]
